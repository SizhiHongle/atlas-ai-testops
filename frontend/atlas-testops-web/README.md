...
| Task | TaskPlan、TaskPlanVersion、TaskRun、ExecutionUnit、Pause/Resume/Cancel |
| Live | UnitAttempt、LiveSnapshot、Takeover/Return/Pause/Resume 与 Fencing |
| Result | ResultSnapshot、FailureCluster、人工 Classification Revision、Task Gate |
| Insight | 7/30/90 天 Brief、Dataset Provenance、InsightSnapshot Pin |

界面中仍禁用的操作均对应后端没有公开契约的能力，例如 AI WorkflowPatch 生成、证据列表/流式日志、完整证据包导出、Feishu OAuth、Identity 聚合创建和 Profile Catalog。前端不会用手填 UUID 或演示行为替代这些 API。
...
- `test:e2e`：桌面和移动端的登录、Task 事实链、Live Control Fencing。
- Visual Regression：Playwright 固定 Task Center 的桌面/移动截图。
