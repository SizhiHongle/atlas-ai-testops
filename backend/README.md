...
- Profile、TaskPlan、TaskPlanVersion、TaskRunManifest、TaskRun、ExecutionUnit、UnitAttempt、TaskUnitExecutionTicket、TaskRunCommandIntent 与 TaskExecutionEvent 的机器 Schema 已导出；P5-00C 已开放 TaskRun / Manifest / Unit / Attempt / Event 的只读公共 API。P5-00D1 通过 `20260717_0027` 为每个 Attempt 增加唯一、secret-free、不可更新删除且强制 RLS 的 Execution Ticket；P5-00D3A 通过 `20260717_0030` 冻结有界基础设施重试策略，只对明确 `INFRA_ERROR` 追加确定性新 Attempt；P5-00D3B 通过 `20260717_0031` 与 `POST /v1/task-runs/{runId}:rerun-infra-failures` 创建数据库证明 exact 选择的新 child TaskRun。P5-00E5 提供正式 `HttpTaskUnitExecutionPort`：只发送 exact Attempt + `ticketId / ticketDigest`，请求和响应均绑定 HMAC、时间窗、Nonce、内容摘要、Worker / Tenant / Attempt / Ticket 与幂等键；生产强制 HTTPS、禁止重定向和环境代理，响应有界且必须 `no-store`。任何传输歧义、非 200、签名或 payload 异常都按结果未知收敛，不会自动重放副作用。真实 SaaS 登录、业务 Operation 与 Result Seal 生成仍由部署端 executor 实现。
...
首个真实 SaaS `PasswordLoginFlow`、生产 Secret Provider / KMS-backed Vault 和真实 SaaS Browser Operation / Route Registry 仍需部署侧提供。生产 Evidence Bucket 的 Object Lock / Versioning、Credential 分离与生命周期策略也属于部署责任。P6-02B2 已完成 UnitAttempt-scoped LiveSession、ControlLease、Epoch / Fence、Human Takeover 与持久化 ActionGrant；P5-00B2B 已提供正式 Root / Attempt Workflow，P5-00D1 已建立不可变 Ticket 与正式 Port 输入授权协议，P5-00E3 已完成 Schedule / CI / Webhook 统一幂等入口，P5-00E4 已完成 100,000-Unit 协议、可恢复分区物化与 Temporal 分页续跑，P5-00E5 已提供可由 CLI / Compose 正式装配的签名 HTTPS production Port。Temporal Schedule 管理、签名外部回调、部署端真实 SaaS executor、容器级 Egress / DNS / UDP / WebRTC 限制、BrowserContext Envelope Key Ring Rotation、公共 Start 自动 Preparation / Bind / Dispatch 与 Multi-actor 尚未完成；缺失时对应能力明确 fail-closed。架构决策见 `../documents/adr/`。
