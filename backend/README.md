...
- P6-02B2 正式 UnitAttempt Live Control：`LiveSession / ControlLease / LiveControlCommand / LiveActionGrant / LiveControlEvent` 全部精确绑定 immutable Execution Ticket 与 Attempt scope；公共 `PAUSE / RESUME / TAKEOVER / RETURN` 使用 Epoch ETag 与幂等异步 Command，Worker 通过 Permit + HMAC 完成初始化、Heartbeat、Safe Point acknowledgement 和 Grant 单次消费/Receipt。Lease TTL 回收会提升 Fence、撤销未消费 Grant、拒绝 pending Command 并进入 `NO_CONTROLLER`；Production Takeover fail-closed，人工影响阻止 `AUTONOMOUS` Seal。
...
- P5-00B1 调度前置边界：四类不可变 `ExecutionProfileVersion` / `IdentityProfileVersion` / `BrowserProfileVersion` / `DataProfileVersion` 正式宿主与发布门禁、`executionProfileVersionId` 统一命名、稳定 Run request digest 与 exact rerun lineage、Run / Attempt 确定性 Temporal identity 与 namespace 全局注册表、`MATERIALIZING → SEALED` 完整性证明、SEALED / Lifecycle / QUEUED Admission、受约束的后续 Attempt、Pending Workflow Start Intent，以及数据库拥有的 Run → Unit → Attempt Revision CAS。`20260718_0042` 保留不超过 64 Units 的原子快路径，并为 65–100,000 Units 增加可认领、可重试、带 Token / Revision Fence 的 64-Unit 物化分区；只有全部分区完整提交后才 Seal 并产生一个 Start Intent。
...
- P5-00B2B Task Worker 装配：真实 `AtlasTaskRunWorkflow` 与 `AtlasUnitAttemptWorkflow` 分别绑定固定 `atlas-task-run` / `atlas-unit-attempt` Queue，Root 与 Attempt Worker 各自具有有界并发。Root 每页最多加载 64 Units、每批最多启动 8 个 Child；大 Run 只在当前页无活跃 Child、无未结算批次且全部结果已落库后 `Continue-As-New`，最后由 PostgreSQL 终态投影收口，不把 100,000 个结果带入单段 History。取消会先排空当前页，并跨续跑保留取消状态。副作用 Activity 固定单次执行、Heartbeat 和等待取消完成，Queue 排队不得越过冻结 deadline；原生取消中的副作用一律按结果未知收敛，Root 保留已完成 Child 的真实结果。数据库 Activity 对瞬时故障耐久 retry、对安全不变量 non-retryable，且所有不可信返回和异常都先转为安全码再写入 Temporal History；Attempt / Run finalize 事件用于 exact replay。Task Worker 和 Intent Consumer 使用独立开关且默认关闭；缺少完整、受信的执行 Adapter 配置时，Task Worker 会在构造数据库或连接 Temporal 前 fail-closed，绝不注册 no-op executor。
- Profile、TaskPlan、TaskPlanVersion、TaskSchedule、TaskRunManifest、TaskRun、ExecutionUnit、UnitAttempt、TaskUnitExecutionTicket、TaskRunCommandIntent 与 TaskExecutionEvent 的机器 Schema 已导出；P5-00C 已开放 TaskRun / Manifest / Unit / Attempt / Event 的只读公共 API。P5-00D1 通过 `20260717_0027` 为每个 Attempt 增加唯一、secret-free、不可更新删除且强制 RLS 的 Execution Ticket；P5-00D3A/D3B 冻结有界基础设施重试并支持数据库证明的 infra-failure child Run。P5-00E5 提供正式双向 HMAC `HttpTaskUnitExecutionPort`；P5-00E6 通过 `20260718_0043` 提供数据库权威 TaskSchedule、fenced Temporal Sync、Pause/Resume 与统一 Schedule fire；P5-00E7 通过 `20260718_0044` 将每个新 TaskGateDecision 原子转换为可恢复、六字段 HMAC 签名的外部 Callback。真实 SaaS 登录、业务 Operation 与 Result Seal 生成仍由部署端 executor 实现。
- P5-00D2A 通过 `20260717_0028` 落地 durable TaskRun `CANCEL`；P5-00D2B 通过 `20260717_0029` 将命令契约升级为兼容旧 Cancel 的 `atlas.task-run-command/0.2`，新增 `PAUSE / RESUME / SUPERSEDED`。`POST /v1/task-runs/{runId}:cancel|pause|resume|rerun-infra-failures` 都要求 `If-Match`、`Idempotency-Key == clientMutationId` 与 `RUN_OPERATOR+`。前三者写 durable command；infra rerun 只接受 `SEALED / CLOSED` source，生成全新 sealed child aggregate与 Start Intent，不向旧 Workflow 发送 RETRY Signal。Task 级控制与 P6-02B2 UnitAttempt Action Safe Point / Takeover 保持独立。
...
- P5-00E2 已开放 `POST /v1/task-plan-versions/{taskPlanVersionId}:run`。Manual compiler 只展开 Case-bound Identity 与 Fixture-bound Data 的兼容组合，协议上限为 100,000 Units；请求的完整 `TaskRetryPolicy` 必须匹配版本冻结的 `infra-retry` digest。不超过 64 Units 继续在请求事务内完成 Manifest v0.2、TaskRun / ExecutionUnit / 首 UnitAttempt、Seal、唯一 `PENDING` Start Intent、Event、Audit / Outbox 与幂等响应；更大 Run 先提交 Root / Manifest / 分区检查点，再由独立 Dispatcher Consumer 可恢复物化和 Seal。exact replay 不重复副作用。
- P5-00E3 已开放统一 `POST /v1/task-runs` Trigger 入口。`SCHEDULE / CI / WEBHOOK` 分别用冻结的 fire、pipeline job 和 delivery 身份生成永久 Trigger Fingerprint，并复用 E2 的 compatible-only 编译、Manifest、Seal、Start Intent、Audit / Outbox 与 exact replay；CI 的 commit / branch 和 Webhook event type 只作为安全审计元数据，不能覆盖 TaskPlanVersion 的环境、凭据、工具或策略。
...
- Insight Brief：`GET http://127.0.0.1:8000/v1/projects/{projectId}/insights/brief?windowDays=30`
- 固定 Insight Snapshot：`POST http://127.0.0.1:8000/v1/projects/{projectId}/insight-snapshots`
- 读取 Insight Snapshot：`GET http://127.0.0.1:8000/v1/insight-snapshots/{snapshotId}`
...
该进程使用独立 Dispatcher DSN，不读取 API Tenant Context，也不能复用 `atlas_app`。它并行交付 Task Start、Task Command、大 Run Materialization 与 Task Schedule Sync Intent；所有 Temporal I/O 均在 Claim 事务外，最终以 Claim Token + Dispatch Revision 的新事务 CAS。Schedule 会精确校验 Memo、Action、Calendar、Timezone、Overlap/Catchup/Jitter，并收敛 Pause 状态。入口和 Compose profile 均默认关闭；只有对应 Worker 与生产执行边界就绪后才可启用。Compose 验证还需要同时显式设置 `ATLAS_TASK_INTENT_CONSUMPTION_ENABLED=true` 并启用 `--profile task-intent-consumer`。

运行独立 Task Schedule Worker：

```bash
ATLAS_TASK_SCHEDULE_WORKER_ENABLED=true \
ATLAS_DATABASE_URL='postgresql://atlas_app:<password>@127.0.0.1:5432/atlas' \
ATLAS_TEMPORAL_NAMESPACE=default \
uv run atlas-task-schedule-worker
```

`POST /v1/task-plan-versions/{id}/schedules` 创建数据库权威 Schedule，`POST /v1/schedules/{id}:pause` 与 `:resume` 使用 `If-Match` 和 Idempotency-Key 修改 desired state。V1 仅支持结构化 Calendar、IANA Timezone、`QUEUE_ONE / SKIP`、有限 Catchup 与 Jitter；API 返回未来五个真实 UTC fire，并明确处理 DST gap/fold。Temporal 保留属性提供名义 fire；Schedule Worker 重读数据库后复用统一 TaskRun compiler。Schedule Worker 和 Compose `task-schedule-worker` profile 默认关闭，不能与 Dispatcher 共用数据库角色。

运行独立 Task Gate Callback Consumer：

```bash
ATLAS_TASK_GATE_CALLBACK_DELIVERY_ENABLED=true \
ATLAS_TASK_DISPATCHER_DATABASE_URL='postgresql://atlas_dispatcher:<password>@127.0.0.1:5432/atlas' \
ATLAS_TASK_GATE_CALLBACK_URL='https://release-gateway.internal/task-gates' \
ATLAS_TASK_GATE_CALLBACK_HMAC_KEY_BASE64='<rotated-base64-key>' \
uv run atlas-task-gate-callback-consumer
```

每个新 `TaskGateDecision` 会在同一事务产生唯一 callback intent；Gate API replay 不重复 intent。HTTP body 固定为 `eventId / taskRunId / manifestHash / gateDecision / timestamp / signature`，其中 eventId 是接收端永久幂等键，HMAC 覆盖其余业务身份与 UTC timestamp。Consumer 只从全局进程配置读取一个 endpoint 和 key，请求与数据库均不能提供 URL；Staging / Production 强制 HTTPS、禁止 redirect 与环境代理。Transport/408/425/429/5xx 使用同一 eventId 有界重投，接收端必须原子去重并对重复事件返回 2xx。入口与 Compose `task-gate-callback-consumer` profile 默认关闭。

运行独立 Task Worker 入口：

```bash
ATLAS_TASK_WORKER_ENABLED=true \
ATLAS_TASK_EXECUTION_API_BASE_URL='https://task-executor.internal' \
ATLAS_TASK_EXECUTION_HMAC_KEY_BASE64='<rotated-base64-key>' \
uv run atlas-task-worker
```

`ATLAS_TASK_WORKER_ENABLED=false` 是默认值，此时进程不会构造数据库或连接 Temporal。CLI 会从完整配置创建 `HttpTaskUnitExecutionPort`；URL / HMAC Key 缺一、Origin 不合法或 Staging / Production 使用 HTTP 时都在任何外部连接前 fail-closed。该 Port 对一个 Attempt 只发起一次有界调用，executor 必须以 Attempt ID 实现同一业务幂等并在返回 `RESULT_FINALIZED` 前写入数据库可复核的 exact ResultRef。仓库不提供 no-op / placeholder 或假 SaaS executor。Root 与 Attempt 使用两个隔离 Worker 和固定 Queue，分别通过 `ATLAS_TASK_RUN_WORKER_MAX_CONCURRENCY` 与 `ATLAS_TASK_ATTEMPT_WORKER_MAX_CONCURRENCY` 限制并发。Compose 还需显式启用 `--profile task-worker` 并注入两项 Adapter 配置，默认服务集合不会启动 Task Worker。
