...
8. `AttemptSeal` 必须归属于 P5 的正式 `UnitAttempt`，因此不在 DebugRun 基础切片中创建无宿主协议；正式任务链落地后复用本 ADR 的合约和证据根。
...
P6-01 已按 [ADR-0008](ADR-0008-database-free-browser-worker.md) 实现不直连主数据库的独立 Browser Worker、受信内部网关、加密 BrowserContext Restore Envelope 与严格报告链。Live、生产 Evidence / Redaction Writer 和正式 AttemptSeal 仍保持在后续切片。
