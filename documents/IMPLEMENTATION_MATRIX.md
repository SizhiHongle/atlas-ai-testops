...
| Unit Attempt | `atlas.unit-attempt/0.1` | `backend/src/atlas_testops/domain/task` | P5-00D3A 对显式 `INFRA_ERROR` 按冻结策略追加确定性、gapless 新 Attempt；Assertion / 产品失败与 `OUTCOME_UNKNOWN` 不自动重试，当前无 AttemptSeal，不能产生 `PASSED` |
...
| 未 Seal 的 Task 执行不能伪装通过 | Workflow / execution port 结果类型无 `PASSED`；`EXECUTED_UNSEALED → FINISHED_UNSEALED` 只持久化为 `INCONCLUSIVE` Quality；失败、歧义、跳过与取消安全收敛；正式 AttemptSeal 待 P6 | P5-00B2B / P6 |
