...
| Task | 任务中心 v0.2 | 四类 profile_version、task_plan/version、task_run、task_run_manifest、execution_unit、unit_attempt、task_run_event、workflow identity registry、durable start intent、`20260716_0025` execution-chain lock、`20260716_0026` 查询索引、`20260717_0027` immutable execution ticket、`20260717_0028` durable command intent、`20260717_0029` pause / resume、`20260717_0030` frozen infra retry policy、`20260717_0031` database-proven infra rerun lineage 与 `20260718_0042` partition checkpoint；Temporal schedule catalog 待后续 | P5-00B1 契约；P5-00B2A Intent Consumer；P5-00B2B Root / Attempt Workflow；P5-00C 查询；P5-00D1 ticket-bound Port；P5-00D2A `CANCEL`；P5-00D2B batch-boundary `PAUSE / RESUME`；P5-00D3A automatic `INFRA_ERROR` retry；P5-00D3B manual infra-failure child Run；P5-00E1 TaskPlan Catalog / immutable publish；P5-00E2 compatible-only Manual Launch；P5-00E3 unified Schedule / CI / Webhook Trigger ingress；P5-00E4 100,000-Unit partition / paged Workflow；P6-02B2 UnitAttempt Takeover；P6-03A trusted ResultRef recovery；P6-03B terminal Result projection | Launch、Task Control 原型保持不变；Live 既有槽位映射真实 Snapshot 与 Takeover / Return | 基础中：100,000-Unit 协议、64-Unit 可恢复分区 / 分页 Workflow、只读查询、immutable Ticket、可靠 Cancel、批次边界 Pause / Resume、有界基础设施自动重试、精确 manual infra rerun、TaskPlan 编写 / 发布、Manual 与统一 Schedule / CI / Webhook Trigger 入口、UnitAttempt Takeover 已验收；P6-03A 只凭数据库 exact ResultRef 允许 `PASSED`，P6-03B 在同一结算事务覆盖 Seal / ClosureNotice 并追加 Unit Resolution。Worker / Consumer 默认关闭且无内置真实 SaaS executor；production Adapter、Temporal Schedule 管理与签名回调尚未实现 |
