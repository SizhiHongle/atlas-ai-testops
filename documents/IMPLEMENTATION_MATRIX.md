...
| Task | 任务中心 v0.2 | 四类 profile_version、task_plan/version、task_run、task_run_manifest、execution_unit、unit_attempt、task_run_event、workflow identity registry、durable start intent、`20260716_0025` execution-chain lock、`20260716_0026` 查询索引、`20260717_0027` immutable execution ticket、`20260717_0028` durable command intent、`20260717_0029` pause / resume、`20260717_0030` frozen infra retry policy 与 `20260717_0031` database-proven infra rerun lineage；schedule 待后续 | P5-00B1 契约；P5-00B2A Intent Consumer；P5-00B2B Root / Attempt Workflow；P5-00C 查询；P5-00D1 ticket-bound Port；P5-00D2A `CANCEL`；P5-00D2B batch-boundary `PAUSE / RESUME`；P5-00D3A automatic `INFRA_ERROR` retry；P5-00D3B manual infra-failure child Run；P5-00E1 TaskPlan Catalog / immutable publish；P5-00E2 compatible-only Manual Launch；P6-03A trusted ResultRef recovery；P6-03B terminal Result projection；takeover 待后续 | Launch、Task Control 原型未改；仅同步生成 API 类型 | 基础中：最多 64 Units 的耐久编排、只读查询、immutable Ticket、可靠 Cancel、批次边界 Pause / Resume、有界基础设施自动重试、精确 manual infra rerun、TaskPlan 编写 / 发布与首次 Manual Launch 已验收；P6-03A 只凭数据库 exact ResultRef 允许 `PASSED`，P6-03B 在同一结算事务覆盖 Seal / ClosureNotice 并追加 Unit Resolution。Worker / Consumer 默认关闭且无内置真实 SaaS executor；production Adapter、Schedule / CI、超过 64 Units 分区化与 Takeover 尚未实现 |
...
| Result | 结果中心 v0.2 | unit_attempt_result_fact、result_ref、result_integrity_incident、attempt_closure_notice、unit_resolution_revision、task_result_snapshot、attempt_fixture_binding、unit_hygiene_resolution_revision、task_result_reevaluation_command、failure_cluster_revision、failure_classification_revision；gate 待后续 | 当前仅内部 Finalize / Projection / Hygiene / Reevaluation / Classification service；results、clusters、gate 公共 API 待后续 | Results 原型未改 | 基础中：P6-03A/P6-03B 已实现 Attempt Fact 与 Unit Resolution；P7-01A 至 P7-01B2 已实现三阶段 Snapshot；P7-02A 已实现 exact Snapshot-bound Cluster、保守 Rule Classification、Evidence Ref 与人工 append-only review。Gate、查询投影和公共 API 未实现 |
...
| Task 副作用不能因自动 retry、replay 或原生取消被误判 | 固定 Root / Attempt Type 与双 Queue；最多 64 Units、8-child batch、deterministic child ID + `REJECT_DUPLICATE`；副作用 Activity `maximumAttempts=1`、Heartbeat 与等待取消完成；`scheduleToClose` 不越过 deadline；数据库 Activity 瞬时故障耐久 retry、确定性不变量 non-retryable；安全码 History、typed fallback 与 exact finalize-event replay；取消时保留已完成 Child，运行中副作用收敛为未知 | P5-00B2B |
| Task Port 不能消费未经授权或被篡改的裸执行输入 | `20260717_0027` 每 Attempt 唯一 immutable Ticket、Scope FK、canonical digest、owner-owned Insert Guard、FORCE RLS 与 SELECT / INSERT 最小权限；Workflow 先 Prepare Ticket，再 Begin / Execute；Port 输入仅为 exact Attempt + `ticketId / ticketDigest`；跨 Tenant、依赖漂移、篡改与重放冲突 fail-closed | P5-00D1 |
