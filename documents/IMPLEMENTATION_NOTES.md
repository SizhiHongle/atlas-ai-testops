...
- P5-00A 已建立正式 `UnitAttempt`；P6-02B2 后续负责 `LiveSession / ControlLease`、浏览器控制 Epoch / Fence、Action Safe Point Pause / Resume / Takeover Command、Human Takeover、Quiesce，以及持久化且绑定控制 Epoch / Fence 的 `ActionGrant`。P5-00D2B 的 Task 级 Pause 只停止新 Unit 派发，不覆盖这些现场能力。B1 SSE 不接收 Frame、Command、Action 或人工输入，不能把只读 Observer 解释为已经实现人工接管。
...
- `atlas-task-intent-consumer` 与 Root / Attempt 双 Worker 使用独立开关，`ATLAS_TASK_INTENT_CONSUMPTION_ENABLED=false`、`ATLAS_TASK_WORKER_ENABLED=false` 且 Compose profiles 默认不启用。P5-00D1 已把 `TaskUnitExecutionPort` 输入收紧为 prepared `ticketId / ticketDigest`，但仓库不提供 no-op、placeholder 或内置 production Adapter；显式启用 Worker 却未注入真实 Adapter 时必须在数据库 / Temporal 连接前 fail-closed。P5-00D2A/D2B 让同一 Consumer 进程并行轮询 Start Intent 与 TaskRun Command Intent，但两类事实各自 Claim / CAS，任何 Temporal I/O 都在事务外；当前公共控制面开放 `CANCEL / PAUSE / RESUME`、command status 与新建 child Run 的 infra-failure rerun。旧 Run RETRY command、Takeover、Schedule / CI Adapter、AttemptSeal、Result 与 Live control 仍未实现。
