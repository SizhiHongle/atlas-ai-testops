...
- `atlas-task-intent-consumer`、Root / Attempt Worker、Schedule Worker 与 Task Gate Callback Consumer 使用独立开关和默认关闭的 Compose profiles。P5-00D1 已把 `TaskUnitExecutionPort` 输入收紧为 prepared `ticketId / ticketDigest`；P5-00E5 已提供可由 CLI / Compose 装配的正式 signed HTTPS Adapter。P5-00D2A/D2B/E4/E6 让同一 Intent Consumer 进程并行轮询 Start Intent、TaskRun Command Intent、大 Run Materialization Partition 与 Schedule Sync Intent；四类事实各自 Claim / CAS，任何 Temporal I/O 都在事务外。P5-00E6 已建立数据库权威 Schedule、结构化 Calendar、IANA Timezone、DST、Overlap/Catchup/Jitter、Pause/Resume、Production 自动 Pause 和统一 Schedule fire。P5-00E7 的独立 Callback Consumer 只持有 dispatcher DSN、一个全局 endpoint 与 HMAC key，HTTP 位于 Claim/Outcome 两个短事务之外。剩余执行面缺口是旧 Run RETRY command 与部署端真实 SaaS executor。
...
- CI 的 commit / branch 与 Webhook event type 只进入允许列表内的审计 Event 元数据，不参与触发身份，也不能覆盖 TaskPlanVersion 冻结的 URL、Environment、Credential、Tool、Model 或 Policy。E6 已完成 Temporal Schedule catalog / overlap / catch-up 管理；E7 已完成 Gate Decision 的签名外部 Callback，但其 endpoint/key 仍必须由部署提供。
...
- P5-00E7 的 `20260718_0044` 让每个新 `TaskGateDecision` 同事务写唯一 `task_gate_callback_intent`。数据库不保存 URL、key、signature、response body 或异常文本；外部 body 只有 `eventId / taskRunId / manifestHash / gateDecision / timestamp / signature`。HMAC 覆盖前五项，timestamp 有回放窗口，eventId 是接收端永久幂等键。Transport/408/425/429/5xx 可用同一 eventId 有界重投；2xx 才标记 `DELIVERED`，其他 4xx 永久 `FAILED`。
- P9 以 `make p9-acceptance` 运行 opt-in 重型门禁并输出 `atlas.p9-acceptance-report/0.1`。本地 Gate 固定包含六类故障注入、100 并发×100 轮 Lease、2× Evidence 参考负载、跨 Project 隔离、30 次黄金事实链、30 个真实 Temporal Schedule 样本和 Live Event P95；任何失败都非零退出。100×100 压力允许 `SKIP LOCKED` Admission 返回短暂 `POOL_EXHAUSTED`，但必须以同一命令身份有界重试、记录背压次数，并最终证明 10,000 次循环冲突 0、Fence 单调、无遗留 Active Lease。
- 本地确定性 Adapter、单机数据库与 Development Temporal 的通过结果不能升级为生产 SLO。月度可用性、真实 Network / Proxy 下 Schedule/Live 延迟、人工 Failure Classification 准确率、真实 SaaS 影子迭代和灾备恢复演练缺少外部证据时必须保持 `NOT_EVALUATED`；全部本地 Gate 通过时总状态仍是 `CONDITIONAL_PASS`。
