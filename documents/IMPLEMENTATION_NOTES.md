...
- `atlas-task-intent-consumer` 与 Root / Attempt 双 Worker 使用独立开关，`ATLAS_TASK_INTENT_CONSUMPTION_ENABLED=false`、`ATLAS_TASK_WORKER_ENABLED=false` 且 Compose profiles 默认不启用。P5-00D1 已把 `TaskUnitExecutionPort` 输入收紧为 prepared `ticketId / ticketDigest`；P5-00E5 已提供可由 CLI / Compose 装配的正式 signed HTTPS Adapter，配置缺失时仍在数据库 / Temporal 连接前 fail-closed。P5-00D2A/D2B/E4 让同一 Consumer 进程并行轮询 Start Intent、TaskRun Command Intent 与大 Run Materialization Partition，三类事实各自 Claim / CAS，任何 Temporal I/O 都在事务外；当前公共控制面开放 `CANCEL / PAUSE / RESUME`、command status、新建 child Run 的 infra-failure rerun，以及 P6-02B2 的 UnitAttempt Live Control。剩余执行面缺口是旧 Run RETRY command、Temporal Schedule 管理、签名外部回调与部署端真实 SaaS executor。
