...
- Run 与 Attempt 的 Temporal Workflow ID 由 Tenant ID + 对象 ID 确定性生成，并写入 `(namespace, workflowId)` 全局 Registry；Start Intent 只是待启动事实。P5-00B2A 的 `STARTED` 只说明 Temporal 接受且其 identity 已通过 Describe 复核，不代表 Task 已运行、成功或产出结果。
...
- `atlas-task-intent-consumer` 使用独立 DSN / 连接池与进程 Settings；`ATLAS_TASK_INTENT_CONSUMPTION_ENABLED=false` 且 Compose profile 默认不启用。P5-00B2A 不注册 no-op / placeholder Workflow；真实 `AtlasTaskRunWorkflow`、Activity、UnitAttempt Workflow、公共 Task API、Schedule / CI Adapter、AttemptSeal、LiveSession、ControlLease、ActionGrant 和 Result Snapshot 仍未实现，生产环境在真实 Task Worker 就绪前必须保持 Consumer 关闭。
