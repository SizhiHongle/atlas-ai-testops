...
| `TaskSchedule` | 数据库权威的周期 TaskPlanVersion desired state | P5-00E6 使用结构化 Calendar、IANA Timezone、V1 Overlap/Catchup/Jitter、Revisioned Pause/Resume、未来五次真实 fire 与 fenced Temporal Sync；Temporal action 仍回到统一 TaskRun compiler |
| `TaskScheduleSyncIntent` | Schedule desired revision 到 Temporal 的可靠同步命令 | API 同事务写入；专用 Dispatcher 在事务外 create/describe/pause/unpause，再用 Claim Token + Dispatch/Schedule Revision CAS；陈旧 Revision 自动 supersede |
...
| `AtlasTaskRunWorkflow` | 一个 sealed TaskRun 的真实 Temporal Root，分页加载 Attempt 计划并耐久聚合 Child 结果 | P5-00E4 已接入最多 100,000 Units 的 64-Unit 分页与安全 Continue-As-New；固定 `atlas-task-run`，按 8-child wave 原子预授权、结算并调度，冻结 backoff 可被控制 Signal 中断 |
...
| `TaskUnitExecutionPort` | UnitAttempt Workflow 调用的受信副作用执行 Adapter 边界 | P5-00E5 已提供 signed HTTPS production Adapter，只发送 `attempt + ticketId + ticketDigest`，双向 HMAC 绑定 exact scope、时间窗、Nonce、Body digest 与幂等键；真实 SaaS executor endpoint、凭据与 Seal 签发由部署侧提供，配置缺失时 Task Worker fail-closed |
