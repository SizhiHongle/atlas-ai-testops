...
- [ADR-0011: 数据库权威 Temporal Task Schedule](adr/ADR-0011-database-authoritative-temporal-task-schedules.md)
- [ADR-0012: 签名 Task Gate Callback 与可靠投递](adr/ADR-0012-signed-task-gate-callbacks.md)
- [ADR-0013: P9 本地参考验收与生产门禁分离](adr/ADR-0013-p9-local-reference-and-production-gates.md)
...
- [P9 本地参考基线](P9_LOCAL_ACCEPTANCE_BASELINE.md)
- [P9 Production Readiness Runbook](runbooks/P9_PRODUCTION_READINESS.md)
...
- P5-00A 已建立 `TaskPlanVersion → TaskRun → ExecutionUnit → UnitAttempt` 正式宿主；P5-00B1 至 P5-00E7 已补齐 Profile、stable request digest、Temporal identity、Seal / CAS、durable Start Intent、100,000-Unit 分区物化 / 分页 Workflow、RBAC + RLS 查询、immutable Execution Ticket、可靠 `CANCEL / PAUSE / RESUME`、frozen infra retry、manual infra-failure child Run、TaskPlan Catalog、compatible-only Manual Launch、统一 Schedule / CI / Webhook Trigger ingress、双向 HMAC 的 signed HTTPS production `TaskUnitExecutionPort`、数据库权威 Temporal Schedule Catalog / Sync / Fire，以及 Gate 事务原子 intent + 固定六字段 HMAC + eventId 幂等重投的外部 Callback。P6 已完成 trusted AttemptSeal / ResultRef、ClosureNotice / UnitResolution 与 UnitAttempt Live Control；P7 已完成三阶段 TaskResultSnapshot、FailureCluster / Classification、fail-closed Gate 和公共 Result API；P8 V1 已完成 comparable Insight Snapshot。Worker 与 Consumer 默认关闭，部署仍须提供真实 SaaS executor 和 Callback endpoint/key，前端原型未修改。
- P9 已新增 opt-in 自动验收和安全 JSON 报告。本地参考基线覆盖六类故障注入、100 并发×100 轮 Lease、2× Evidence 负载、跨 Project 隔离、30 次黄金链、30 个真实 Temporal Schedule 样本与 100 个 Live 样本；全部本地 Gate 通过。月度可用性、人工分类准确率、真实影子迭代与灾备演练没有外部证据，报告保持 `CONDITIONAL_PASS`，不伪装为生产验收完成。
