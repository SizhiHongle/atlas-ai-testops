...
- P5-00A 已建立 `TaskPlanVersion → TaskRun → ExecutionUnit → UnitAttempt` 正式宿主；P5-00B1 至 P5-00C 已补齐 Profile、稳定 request digest、Temporal identity、Seal / CAS、durable Start Intent、最多 64 Units 的 Root / Attempt Workflow 与 RBAC + RLS 只读查询。P5-00D1 通过 `20260717_0027` 增加每 Attempt 唯一、不可变、secret-free 的 Execution Ticket；P5-00D2A/D2B 通过 `20260717_0028/0029` 增加可靠 `CANCEL / PAUSE / RESUME`；P5-00D3A/D3B 已增加 frozen infra retry 与数据库证明 exact selection 的 manual infra-failure child Run；P5-00E1 已开放 TaskPlan 创建与不可变发布，P5-00E2 已开放 compatible-only 首次 Manual Launch，并原子创建 Manifest、最多 64 Units、首 Attempt、Seal 与 Start Intent。P6-03A 已让 Task Workflow 只在读取到 exact `AttemptSeal + ResultRef` 后表达 `PASSED`；P6-03B 已让无 Seal 终态写入 ClosureNotice，并以追加式 UnitResolutionRevision 解析全部 Attempt 历史；P7-01A 至 P7-01B2 已建立三阶段 TaskResultSnapshot；P7-02A 已建立 exact Snapshot-bound FailureCluster、保守 Rule Classification、Evidence Ref 与人工 append-only review。Worker 与 Consumer 默认关闭，仓库仍没有内置真实 SaaS production Adapter；Gate 与公共 Result API 尚未开放，前端原型未修改。
- P6-03A 已建立 Ed25519 `AttemptSeal`、稳定 `ResultRef`、append-only Integrity Incident、原子 Finalize 与回包丢失恢复边界；P6-03B 已建立 ClosureNotice、完整终态覆盖和可重建 Unit Resolution Revision 链。正式 `LiveSession`、`ControlLease`、浏览器控制 Epoch / Fence、Human Takeover 与持久化 ActionGrant 仍属于 P6-02B2；当前 SSE 只是只读 Observer，不提供 Action 或人工控制。真实 SaaS Operation / Route Registry 和容器网络沙箱同样继续 fail-closed。
- TaskPlan、TaskPlanVersion、Manual Launch、Task 执行对象、四类 Profile、Task Unit Execution Ticket、AttemptSeal、ResultRef、AttemptClosureNotice、UnitResolutionRevision、TaskResultSnapshot、TaskResultReevaluationCommand、AttemptFixtureBinding、UnitHygieneResolutionRevision、FailureClusterRevision 与 FailureClassificationRevision 机器 Schema 已导出；Gate 和 Insight Schema 会在对应领域代码落地时增加，不提前创建空契约。
