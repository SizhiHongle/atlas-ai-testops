...
- P5-00A 已建立 `TaskPlanVersion → TaskRun → ExecutionUnit → UnitAttempt` 正式宿主；P5-00B1 至 P5-00E5 已补齐 Profile、stable request digest、Temporal identity、Seal / CAS、durable Start Intent、100,000-Unit 分区物化 / 分页 Workflow、RBAC + RLS 查询、immutable Execution Ticket、可靠 `CANCEL / PAUSE / RESUME`、frozen infra retry、manual infra-failure child Run、TaskPlan Catalog、compatible-only Manual Launch、统一 Schedule / CI / Webhook Trigger ingress，以及双向 HMAC 的 signed HTTPS production `TaskUnitExecutionPort`。P6 已完成 trusted AttemptSeal / ResultRef、ClosureNotice / UnitResolution 与 UnitAttempt Live Control；P7 已完成三阶段 TaskResultSnapshot、FailureCluster / Classification、fail-closed Gate 和公共 Result API；P8 V1 已完成 comparable Insight Snapshot。Worker 与 Consumer 默认关闭，部署仍须提供真实 SaaS executor；Temporal Schedule 管理与签名外部回调继续 fail-closed，前端原型未修改。
