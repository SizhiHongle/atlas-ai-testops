...
- 后端已接入 PostgreSQL/Psycopg 连接池、Alembic、RLS、Transactional Outbox、幂等、Platform RBAC、Argon2id 与 Opaque Session、测试账号目录、ConnectorInstallation / Capability Snapshot、账号登录身份与角色健康检查、Lease/Fencing、一次性 Secret Grant、独立 Auth Session Worker、AES-GCM SessionArtifact Vault、Fixture 耐久运行与发布证据、TestCase / WorkflowDraft / DebugRun / CaseVersion 发布闭环、TaskPlan Catalog 与 Manual / Schedule / CI / Webhook 统一 Trigger ingress，以及 P6 可信执行/证据事实链和 UnitAttempt Live Control、P7 immutable Result Snapshot、Failure Classification、三值 Task Gate 和公开 Result 查询/API、P8 V1 comparable Insight Brief / DatasetCut / immutable Snapshot。Browser Worker 通过短期 Permit + HMAC 内部网关、Temporal Activity、加密 BrowserContext Envelope、严格 Report Hash-chain 和 Epoch/Fence 绑定的单次 ActionGrant 执行冻结 Plan，不直接访问主数据库。
